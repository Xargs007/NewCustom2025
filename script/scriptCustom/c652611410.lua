-- Dolor de Pesadilla, fix by Xargs0x1 2025
local s,id=GetID()
function s.initial_effect(c)
	...
	-- Opponent's monsters must attack your "Yubel"
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_FIELD)
	e2:SetCode(EFFECT_MUST_ATTACK)
	e2:SetRange(LOCATION_SZONE)
	e2:SetTargetRange(0,LOCATION_MZONE)
	e2:SetCondition(s.yubel_condition)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	e3:SetCode(EFFECT_MUST_ATTACK_MONSTER)
	e3:SetValue(s.yubel_filter)
	c:RegisterEffect(e3)

	-- Opponent takes battle damage instead of controller
...
function s.yubel_filter(c)
	return c:IsSetCard(SET_YUBEL)
end

function s.yubel_condition(e)
	return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsSetCard,SET_YUBEL),e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil)
end

-- Target for the search effect
...
-- Operation for the search effect
